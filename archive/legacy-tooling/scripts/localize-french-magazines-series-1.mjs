...
import { FRENCH_MAGAZINE_SERIES_1 as translations,FRENCH_PUBLICATION_DESCRIPTIONS,FRENCH_PUBLICATION_LABELS } from "./data/french-magazine-series-1.mjs";

const moduleId="fallout2d20-compendium";
async function load(pack,language){const dir=path.join("src","packs",language,`${pack}.db`),entries=[];for(const filename of await readdir(dir)){if(!filename.endsWith(".json"))continue;entries.push({filename,file:path.join(dir,filename),document:JSON.parse(await readFile(path.join(dir,filename),"utf8"))});}return entries;}
...
for(const pack of ["perks","books-and-magazines"]){
  ...
  for(const entry of french){const source=enById.get(entry.document._id),translation=translations[source?.name];if(!translation)continue;
    ...
    else{doc.system.publication=FRENCH_PUBLICATION_LABELS[translation.publication];doc.system.description=`<p>${FRENCH_PUBLICATION_DESCRIPTIONS[translation.publication]}</p><p><strong>Aptitude :</strong> ${translation.effect}</p>`;doc.system.effect=`<p>${translation.effect}</p>`;}
...
console.log(`Localized ${Object.keys(translations).length} magazine issues and paired perks in French.`);
