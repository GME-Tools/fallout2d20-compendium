...
test("Core hit-location tables preserve exact d20 coverage and bilingual results", async () => {
  const expected = {
    en: {
      ...
      "Mister Handy Hit Locations": [[1, 2, "Optics (injury as per Head)"], [3, 8, "Main body (injury as per Torso)"], [9, 11, "Arm 1"], [12, 14, "Arm 2"], [15, 17, "Arm 3"], [18, 20, "Thruster (injury as per Leg)"]],
  ...
  for (const language of ["en", "fr"]) {
    ...
    for (const [name, rows] of Object.entries(expected[language])) {
      ...
      assert.equal(table.formula, "1d20");
      if (!["Hit Locations", "Localisation des dégâts", "Mister Handy Hit Locations", "Localisation des dégâts sur Mister Handy"].includes(name)) assert.deepEqual(table.flags["fallout2d20-compendium"].source.errata, ["errata-v6-2026"]);
