...
test("artwork inventory contains each canonical identity exactly once with valid EN/FR references", async () => {
...
test("artwork inventory serialization is deterministic", async () => {
  const text = await readFile(INVENTORY_PATH, "utf8");
  assert.equal(serializeInventory(parseInventory(text)), text);
});

test("resolved owner annotations remain in durable source data", async () => {
  const durable = await readFile("scripts/data/owner-artwork-annotations.mjs", "utf8");
