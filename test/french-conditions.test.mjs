...
async function load(language,pack){const dir=path.join("src","packs",language,`${pack}.db`);return new Map(await Promise.all((await readdir(dir)).filter(f=>f.endsWith(".json")).map(async f=>{const d=JSON.parse(await readFile(path.join(dir,f),"utf8"));return[d._id,d]})));}
