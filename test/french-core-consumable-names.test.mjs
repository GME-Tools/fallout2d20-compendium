...
test("every Core consumable has either an official French name or a documented doubt",async()=>{
  const root="src/packs/fr/consumables.db",documents=await Promise.all((await readdir(root)).filter(file=>file.endsWith(".json")).map(file=>readFile(path.join(root,file),"utf8").then(JSON.parse)));
  const roots=documents.filter(document=>/^!items![^.!]+$/.test(document._key??"")),covered=new Set([...Object.keys(FRENCH_CORE_CONSUMABLE_NAMES),...Object.keys(FRENCH_CORE_CONSUMABLE_NAME_DOUBTS)]);
