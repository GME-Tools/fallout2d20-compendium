...
const names = JSON.parse(await readFile("catalog/v1-core-energy-weapon-mod-fr-names.json", "utf8"));
...
for (const file of await readdir(sourceRoot)) {
  ...
  if (source.system.weaponType !== "energyWeapons") continue;
  const frenchName = names[source.name];
  if (!frenchName) throw new Error(`Missing official French Energy Weapons mod name: ${source.name}`);
  ...
  document.flags[MODULE_ID].source = {
    book: SOURCE_ID, language: "fr", page: "104-105", translatedFrom: document._id,
...
console.log(`Imported ${count} French Energy Weapons modifications.`);
