...
for (const row of rows) if (row.current_status !== "placeholder" && row.current_artwork_source) {
  ...
  Object.assign(row,{
    candidate_source_kind:url?"owner-approved-web":row.current_artwork_source.startsWith("AI-generated")?"generated-private":row.current_artwork_source.startsWith("Fallout: The Roleplaying Game Starter Set")?"owned-official-pdf":row.current_artwork_source.startsWith("US-202 reuse")||row.current_artwork_source.startsWith("shared from sole user actor")?"repository-reviewed-asset":row.current_artwork_source.startsWith("Owner-supplied repository asset")?"owner-supplied-game-asset":row.candidate_source_kind,
...
const outputs = contactSheetOnly
  ? [[CONTACT_SHEET_PATH, contact]]
  : [[INVENTORY_PATH, inventory], [CONTACT_SHEET_PATH, contact]];
if (check) {
  ...
  console.log(contactSheetOnly
    ? `Generated artwork contact sheet (${rows.length} unique canonical pairs; reviewed inventory unchanged).`
    : `Generated artwork inventory and contact sheets (${rows.length} unique canonical pairs).`);
}
