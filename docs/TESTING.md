...
when the active story requires it.

Generated outputs are `generated/source-packs/`, `packs/`, and `dist/`. They are not
editable sources. Reviewed inventories are manual inputs and must not be regenerated
implicitly by validation.
...
4. use `./ci/run_headless_validation.sh` and the browser validator when the story
   requires runtime qualification;
5. record qualification only after the tested commit itself passes.

The maintained workstation Foundry server path is
`/home/benjamin/Foundry/foundryapp`. Runtime compatibility is defined in
